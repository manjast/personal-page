#!/usr/bin/env node
// Compose 5 contact sheets (3x2 grid) from the 30 rendered PNGs
const puppeteer = require('puppeteer-core');
const path = require('path');
const fs = require('fs');

const variants = [
  { key: 'a', title: 'A — Operator\'s Notebook' },
  { key: 'b', title: 'B — Engineering Postmortem' },
  { key: 'c', title: 'C — Typewriter Spec' },
  { key: 'd', title: 'D — Lab Notebook' },
  { key: 'ac', title: 'A+C — Notebook + Typewriter' },
];
...
<body>
  <h1>${v.title}</h1>
  <div class="sub">Contact sheet — 6 slides (1080x1350 each) — for visual comparison</div>
  <div class="grid">
    ...
    const outPath = path.join(__dirname, 'png', `contact-sheet-${v.key}.png`);
